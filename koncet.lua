local FIREBASE_URL = "https://mengfirebase-63df4-default-rtdb.firebaseio.com/"
...
local eventList = {
    ...
    "Toxic Boil", "Flower Guardian Hunt", "Rotbloom Hunt"
}
...
local function getWeatherTotem()
    local currentWorldStatus = ""
    pcall(function()
        currentWorldStatus = game:GetService("Players").LocalPlayer.PlayerGui.hud.safezone.worldstatuses["3_weather"].label.Text
    end)
...
-- ============================================================
-- SUNKEN HELPER — return label string + sisa detik
-- ============================================================
local function getNextSunkenIn(uptimeSec)
    ...
    local s = diffSec % 60
    return string.format("%dM %dS", m, s), diffSec
end
...
local function SF_Fetch()
    ...
    -- Sort: sunken terdekat dulu, kalau sama baru sort by jumlah event
...
SearchBox.PlaceholderText        = "Search by event, weather, sunken..."
...
local function buildCard(s, idx)
    ...
    local sunkenStr, sunkenSec = getNextSunkenIn(s.uptimeSeconds)
    local isSunkenSoon   = sunkenSec and sunkenSec <= 300   -- < 5 menit
    local isSunkenClose  = sunkenSec and sunkenSec <= 600   -- < 10 menit
    ...
    -- Border: cyan kalau sunken soon, orange kalau sunken close, default kalau biasa
    if isSunkenSoon then
        stroke(Card, C.Cyan, 1.5)
    elseif isSunkenClose then
        stroke(Card, C.Orange, 1.5)
    else
    ...
    -- Accent left bar
    if hasEvents or isSunkenSoon or isSunkenClose then
        local barColor = isSunkenSoon and C.Cyan or (isSunkenClose and C.Orange or C.Accent)
    ...
    EvLbl.Size                   = UDim2.new(1, -160, 0, 22)
    EvLbl.Position               = UDim2.new(0, 14, 0, 14)
    ...
    InfoLbl.Position               = UDim2.new(0, 14, 0, 42)
    InfoLbl.BackgroundTransparency = 1
    InfoLbl.Text                   = table.concat(infoParts, sep)
    InfoLbl.TextColor3             = isSunkenSoon and C.Cyan or (isSunkenClose and C.Orange or C.TextDim)
    InfoLbl.TextSize               = 11
    ...
    -- Badge SUNKEN SOON / SOON
    if isSunkenSoon then
        local Badge = Instance.new("TextLabel")
        Badge.Size                   = UDim2.new(0, 88, 0, 18)
        Badge.Position               = UDim2.new(1, -164, 0, 10)
        Badge.BackgroundColor3       = C.CyanBG
        Badge.Text                   = "SUNKEN SOON"
    ...
    elseif isSunkenClose then
        ...
        Badge.Position               = UDim2.new(1, -148, 0, 10)
        Badge.BackgroundColor3       = C.OrangeBG
        Badge.Text                   = "10M SUNKEN"
        Badge.TextColor3             = C.Orange
        Badge.TextSize               = 9
        Badge.Font                   = Enum.Font.GothamBold
        Badge.ZIndex                 = 45
        Badge.Parent                 = Card
    ...
    task.spawn(function()
        ...
        for _, s in ipairs(servers) do
            if query == "" then
                ...
                -- Cek "sunken" keyword
                if not matched and query:find("sunken", 1, true) then
                    local _, sec = getNextSunkenIn(s.uptimeSeconds)
                    if sec and sec <= 600 then
                        matched = true
                    end
                end
