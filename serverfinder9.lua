...
local function buildCard(s, idx)
    ...
    InfoLbl.Text = string.format("%d / %d players  ·  <font color='%s'>Starfall: %s</font>  ·  %s",
        s.playerCount or 0, 
        s.maxPlayers or 20, 
    ...
    task.spawn(function()
        ...
        for _, s in ipairs(servers) do
            if query == "" then
                ...
                if not match and s.weather and s.weather:lower():find(query, 1, true) then
                    match = true
                end
                
                if not match and s.starfall and "starfall":find(query) then
                    match = true
                end
